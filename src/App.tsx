...
const T={navy:"#0A1628",blue:"#152845",gold:"#C8973A",gold2:"#F0C060",white:"#FFFFFF",gray:"#8A9BB0",green:"#27AE60",teal:"#16A085",red:"#C0392B"};
...
function MatchRow({match,predHome,predAway,predWinner,realHome,realAway,onPredChange,adminMode,onRealChange}:any) {
  ...
  return (
    <div className="match-row">
      <div className="team-name home" style={{color:rW==="home"?T.gold2:T.white}}>{match.home}</div>
      ...
      <div className="team-name" style={{color:rW==="away"?T.gold2:T.white}}>{match.away}</div>
    ...
    <div>
      ...
      <div className="card">
        <div className="group-header">
          ...
          <div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.05rem",letterSpacing:1}}>GRUPO {group.id}</div><div style={{color:T.gray,fontSize:".73rem"}}>{group.teams.join(" · ")}</div></div>
        ...
        return (
          <div key={sp.id} className="special-card" style={{border:realVal?(correct?`1.5px solid ${T.green}`:`1.5px solid ${T.red}`):"1px solid rgba(200,151,58,.2)"}}>
            ...
            <div>
              ...
              {realVal&&<div style={{fontSize:".73rem",marginTop:".28rem",color:T.gray}}>Real: <strong style={{color:correct?T.green:T.red}}>{realVal}</strong></div>}
            </div>
            <div style={{textAlign:"center"}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.05rem",color:T.gold2}}>{sp.pts}pts</div>
              {realVal&&<div style={{fontSize:".68rem",color:correct?T.green:T.red}}>{correct?"✓ +"+sp.pts:"✗"}</div>}
            </div>
          </div>
        );
      })}
...
function AdminPanel({results,adminData,participants,onSaveResults,onSaveAdminData}:any) {
  ...
  if (!isAdmin) return (
    <div className="card" style={{maxWidth:360,margin:"0 auto"}}>
      ...
      <p style={{color:T.gray,fontSize:".83rem",marginBottom:"1rem",textAlign:"center"}}>Solo el administrador puede cargar resultados.</p>
  ...
  return (
    <div>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        <div style={{fontSize:".8rem",color:T.gray,marginBottom:".5rem",fontWeight:700}}>👥 PARTICIPANTES ({Object.keys(participants||{}).length})</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:".35rem"}}>
          {Object.keys(participants||{}).map(n=><div key={n} style={{background:"rgba(200,151,58,.1)",border:`1px solid ${T.gold}`,borderRadius:6,padding:".25rem .65rem",fontSize:".78rem",color:T.gold2}}>{n}</div>)}
        </div>
      </div>
      ...
      {adminTab==="groups"&&<div>
        ...
        <div className="card">
          <div className="group-header" style={{marginBottom:".7rem"}}><div className="group-letter">{group.id}</div><div><div style={{fontFamily:"'Barlow Condensed',sans-serif"}}>Grupo {group.id}</div><div style={{color:T.gray,fontSize:".73rem"}}>{group.teams.join(" · ")}</div></div></div>
      ...
      {adminTab==="specials"&&<div>
        <div className="card">
          <div style={{fontFamily:"'Bebas Neue',sans-serif",color:T.gold2,marginBottom:".9rem"}}>Resultados Reales — Nivel 3</div>
          {SPECIALS.map(sp=>(
            <div key={sp.id} className="special-card" style={{marginBottom:".5rem"}}>
              ...
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.05rem",color:T.gold2,textAlign:"center"}}>{sp.pts}pts</div>
            </div>
          ))}
        </div>
...
export default function App() {
  ...
  if(!user) return (
    <div className="app">
      <style>{css}</style>
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"1.5rem",background:`linear-gradient(160deg,${T.navy} 0%,${T.blue} 60%,${T.navy} 100%)`}}>
        <div style={{textAlign:"center",marginBottom:"1.8rem"}}>
          <div className="hero-stars">★ ★ ★ ★ ★</div>
          <div className="hero-badge">PRODE OFICIAL</div>
          <h1 className="hero-title">MUNDIAL<br/>2026</h1>
          <p style={{color:T.gray,fontSize:".88rem",marginTop:".5rem"}}>Estados Unidos · México · Canadá</p>
          <p style={{color:T.gray,fontSize:".75rem",marginTop:".2rem"}}>11 junio – 19 julio · 48 selecciones</p>
        </div>
        <div className="card card-gold" style={{maxWidth:370,width:"100%"}}>
          ...
          <div style={{marginBottom:".7rem"}}>
            <label style={{fontSize:".76rem",color:T.gray,marginBottom:".3rem",display:"block"}}>NUEVO PARTICIPANTE</label>
          ...
          {Object.keys(participants).length>0&&<>
            ...
            <div style={{fontSize:".73rem",color:T.gray,marginBottom:".45rem"}}>YA ESTOY REGISTRADO:</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:".35rem"}}>
        ...
        <p style={{marginTop:"1rem",fontSize:".7rem",color:T.gray,textAlign:"center",maxWidth:300}}>Tus predicciones se guardan en tiempo real y son compartidas con todos.</p>
      </div>
    </div>
  );
    ...
    <div className="app">
      ...
      <nav className="nav">
        ...
        <div style={{fontSize:".75rem",color:T.gray}}>
          ...
          <button style={{background:"none",border:"none",color:T.gray,cursor:"pointer",fontSize:".72rem"}} onClick={()=>setUser(null)}>↩</button>
        </div>
      </nav>
        ...
        {tab==="home"&&(
          <div>
            <div style={{textAlign:"center",padding:"1.5rem 0 1rem",position:"relative"}}>
              <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 50% 0%,rgba(200,151,58,.1) 0%,transparent 70%)",pointerEvents:"none"}}/>
              <div className="hero-stars">★ ★ ★ ★ ★</div>
              <h1 className="hero-title" style={{fontSize:"clamp(2.3rem,5vw,3.8rem)"}}>MUNDIAL 2026</h1>
              <p style={{color:T.gray,marginTop:".3rem"}}>Hola, <strong style={{color:T.gold2}}>{user}</strong> — ¡Que gane el mejor!</p>
            </div>
            <div className="grid-3" style={{marginBottom:"1.3rem"}}>
              {[{label:"Tu posición",val:myRank||"–",sub:"en la tabla"},{label:"Tus puntos",val:myScore.total,sub:`N1:${myScore.n1} N2:${myScore.n2} N3:${myScore.n3}`},{label:"Participantes",val:Object.keys(participants).length,sub:"jugando"}].map(({label,val,sub})=>(
                <div key={label} className="card card-gold" style={{textAlign:"center",padding:"1rem .7rem"}}>
                  <div style={{fontSize:".68rem",textTransform:"uppercase",letterSpacing:"1px",color:T.gray,marginBottom:".25rem"}}>{label}</div>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"2.6rem",color:T.gold2,lineHeight:1}}>{val}</div>
                  <div style={{fontSize:".7rem",color:T.gray,marginTop:".18rem"}}>{sub}</div>
                </div>
              ))}
            </div>
            <div className="card" style={{marginBottom:"1rem"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:".7rem"}}>
                {[{n:"1",label:"Básico",color:T.green,max:"~70pts",items:["Ganador grupos · 1pt","Ganador elim. · 2pts"]},{n:"2",label:"Intermedio",color:"#6B9FD4",max:"~120pts",items:["Exacto grupos · 3pts","Exacto elim. · 5pts","1°+2° grupo · 4pts","3° · 2pts"]},{n:"3",label:"Experto",color:T.gold,max:"~45pts",items:["Campeón · 15pts","Subcampeón · 10pts","Goleador · 8pts","Arquero · 6pts","MVP · 6pts"]}].map(lv=>(
                  <div key={lv.n} style={{padding:".7rem",borderRadius:10,border:`1.5px solid ${lv.color}44`,background:`${lv.color}12`}}>
                    ...
                    {lv.items.map(i=><div key={i} style={{fontSize:".72rem",color:T.gray}}>· {i}</div>)}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <div className="card" style={{marginBottom:"1rem"}}>
              <div style={{display:"grid",gridTemplateColumns:"2.2rem 1fr 3rem 3rem 3rem 3.5rem",gap:".4rem",padding:".35rem .9rem",borderBottom:"1px solid rgba(200,151,58,.2)",marginBottom:".55rem"}}>
                {["#","Nombre","N1","N2","N3","Total"].map(h=><div key={h} style={{fontSize:".68rem",fontWeight:700,textTransform:"uppercase",letterSpacing:"1px",color:T.gray,textAlign:"center"}}>{h}</div>)}
              </div>
              {leaderboard.length===0&&<div style={{textAlign:"center",color:T.gray,padding:"2rem"}}>Aún no hay puntos.</div>}
              {leaderboard.map(({name,n1,n2,n3,total}:any,i:number)=>(
                <div key={name} className={`lb-row ${i===0?"lb-gold":i===1?"lb-silver":i===2?"lb-bronze":"lb-norm"}`} style={{border:name===user?`2px solid ${T.gold2}`:undefined}}>
                  <div className="lb-rank" style={{color:i===0?T.gold:i===1?T.gray:i===2?"#C07840":T.gray}}>{i===0?"🥇":i===1?"🥈":i===2?"🥉":i+1}</div>
                  <div className="lb-name" style={{color:name===user?T.gold2:T.white}}>{name}{name===user?" 👈":""}</div>
                  <div className="lb-pts" style={{color:T.green}}>{n1}</div>
                  <div className="lb-pts" style={{color:"#6B9FD4"}}>{n2}</div>
                  <div className="lb-pts" style={{color:T.gold}}>{n3}</div>
                  <div className="lb-total">{total}</div>
                </div>
              ))}
            </div>
            <div style={{textAlign:"center",fontSize:".73rem",color:T.gray}}>Máximo posible: ~235 pts · Se actualiza en tiempo real</div>
          </div>
        )}
